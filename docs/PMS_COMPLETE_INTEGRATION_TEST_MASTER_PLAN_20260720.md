...
| E2E-026 | 전용 QA 호텔 초기화 | Admin 입점 계약→권한→점검 차단/복구→개인·단체 예약/체크인→5종 부가서비스·출력물→비품 구매→수납·정산·되돌리기→체크아웃→청소→CRM/VIP·단체통계→마감·감사 | `PMS_FULL_CYCLE_E2E_SCENARIO_20260722.md`의 상태·금액·시재·감사 로그 전체 일치 |

기존 부록 B의 E2E-001~025는 모두 유지한다. 위 핵심 경로는 가장 위험한 실행 순서를 고정한 것이며, 나머지 케이스도 동일 PASS 게이트로 실행한다.
