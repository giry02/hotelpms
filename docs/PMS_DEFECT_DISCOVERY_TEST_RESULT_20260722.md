...
| DISC-RSL-005 | 두 탭에서 동일 지출을 동시에 열고 각각 111·222로 저장 | 두 번째 값 222가 충돌 경고 없이 첫 번째 값 111을 덮어씀. 서버 버전 또는 조건부 갱신이 없어 충돌 안전성을 보장할 수 없음 | BLOCKED | `outputs/extended-validation-20260722/resilience-persistence.json` |
...
| 예약 회귀 | 26/26 PASS, 기존 체크아웃 상태 8개와 신규 처리 상태 9개 PASS |
| 단체 회귀 | 17/17 PASS |
| 업무 E2E | 12/12 PASS |
| 인증 | 4/4 PASS |
| 프런트 데스크 | 7/7 PASS |
...
현재 배포본은 정적 프런트엔드와 브라우저 저장소 기반 데모 환경이다. 브라우저 종료 후 데이터 유지와 순차 저장은 검증했지만, 동일 데이터를 두 탭에서 동시에 수정하면 마지막 저장이 이전 값을 경고 없이 덮어쓴다. 실제 서버 DB와 인증 API가 연결되면 서버 트랜잭션, 버전 번호 또는 ETag 기반 조건부 갱신, 네트워크 재시도, 권한 재검증을 동일 케이스로 다시 실행해야 한다.

## 13. 예약 처리 능동 탐색 추가 결과

사용자가 제시한 단일 조기 체크아웃 사례만 수정하지 않고, 반대 분기와 비정상 조작을 포함한 9개 상태 전이를 추가했다.

| ID | 실제 입력·조작 | 수정 전 | 수정 후 |
|---|---|---|---|
| UREG-RES-001 | 미수금 PHP 25인 투숙객 체크아웃 | 차단 | PASS: 상태 불변, 모달 유지, 감사 0건 |
| UREG-RES-002 | 조기 체크아웃 확인 취소 | 취소 | PASS: 투숙·객실 상태 유지 |
| UREG-RES-003 | 청소 필요 객실 체크인 경고 취소 | 취소 | PASS: 예약·객실·감사 상태 불변 |
| UREG-RES-004 | 청소 필요 객실 경고 확인 후 체크인 | 확인 | PASS: 저장·감사 각 1건, 모달 닫힘 |
| UREG-RES-005 | 객실 이동 후 체크아웃 | 확인 | PASS: 현재 객실만 공실·청소 필요로 변경 |
| UREG-RES-006 | 익일 예약 체크인 | 실행 | PASS: 차단, 상태·감사 불변 |
| UREG-RES-007 | 체크아웃 동시 2회 호출 | FAIL: 확인 2회, 저장 2회, 하우스키핑 동기화 2회, 감사 로그 2건 | PASS: 모든 부수 효과 1회 |
| UREG-RES-008 | 완료 예약 체크아웃 재실행 | 실행 | PASS: 추가 저장·감사 0건 |
| UREG-RES-009 | 저장소 폴백 체크아웃 후 새로고침 | 실행 | PASS: 완료 시각·객실 상태 유지 |

### DEF-20260722-003 예약 처리 버튼 중복 실행

- 등급: P1 상태·감사 로그 중복 가능성
- 재현: 체크아웃 버튼을 빠르게 두 번 실행하거나 첫 요청 완료 전에 동일 함수를 다시 호출
- 수정 전: 확인창, 예약 저장, 하우스키핑 동기화, 감사 로그가 각각 2회 실행
- 원인: 화면 버튼 비활성화와 함수 수준 재진입 잠금이 없어서 비동기 확인창 대기 중 두 번째 실행이 진입
- 수정: 예약 ID와 처리 동작을 키로 하는 실행 잠금을 추가하고, 처리 중 예약 처리 버튼을 비활성화하며 `finally`에서 잠금과 버튼을 복구
