...
| UAT-REG-008 | closed Folio인데 잔액이 남아 정산/마감 기준이 모순됨 | 미수금 또는 완료대기 상태여야 할 Folio를 closed 상태로 둠 | closed Folio는 잔액 0이어야 하며, 잔액이 있으면 open/unpaid 상태인지 확인 | 수정/자동검증 |
...
3. 잔액이 남은 Folio는 `open` 또는 `unpaid` 계열 상태로 분리한다.
4. 시연용 데이터는 KPI, 목록, 상세, 로그 화면에서 같은 기준으로 계산되는지 확인한다.
