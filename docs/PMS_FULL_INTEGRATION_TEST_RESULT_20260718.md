...
- 페이지별 상세 결과: `docs/PMS_FULL_INTEGRATION_TEST_CASE_RESULT_20260719.md`
- 프로세스 기준서: `docs/PROCESS_TEST_STANDARD.md`
- 실행 환경: Windows, Node.js `v26.2.0`, Playwright Chromium, 로컬 정적 서버 `http://127.0.0.1:8765`

## 1. 판정

이번에 실행한 공통 자동화 묶음은 **PASS**다. 페이지 로드, 입력 필드, 버튼/팝업, 다국어, 저장소 쓰기, 핵심 업무 흐름, 예약/단체 회귀, 중요 UI 시나리오에서 실패가 남아 있지 않다.

다만 이 결과를 계획서 673개 케이스 전체의 PASS로 판정할 수는 없다. 특히 계획서 7장의 Hotel PMS 416건을 1:1로 다시 대조한 결과는 **PASS 15 / PARTIAL 33 / NOT RUN 368**이다. 이번 결과는 60개 활성 HTML 페이지의 공통 자동화 감사와 위험 기반 핵심 시나리오 실행 결과이며, 페이지별 전체 기능 테스트는 아직 `부분 실행` 상태다. 실물 프린터, 실제 결제·메일 외부 연동, Samsung Internet/Safari 실기기 검증도 별도 수동 검증이 필요하다.
...
| 자동화 ID | 시나리오 | 자동화 실행 | 계획서 판정 |
|---|---|---:|---:|
| STAFF-002 | 직원 편집 버튼 단일 클릭과 영문 라벨 | PASS | PASS |
| STAFF-004 | 신규 직원 폼 필드 구성과 영문 라벨 | PASS | PARTIAL |
| EXP-006-PARTIAL | 기존 비품 구매 값 로드 | PASS | PARTIAL |
| EXP-CRUD-TRACE | PHP/USD/KRW 선택 및 임시 건 등록·수정·삭제 | PASS | EXP-003/006/007 PARTIAL |
| EXP-009-PARTIAL | 사용자 지정 시작일·종료일 입력값 유지 | PASS | PARTIAL |
| RES-BOARD-002-PARTIAL | 선택 객실과 신규 예약 팝업 기본값 일치 | PASS | PARTIAL |
| RES-BOARD-003-PARTIAL | 청소 미완료 경고와 취소 시 미저장 | PASS | PARTIAL |
| RES-BOARD-005 | 1203호 선택, 팝업 1203, 저장 room 1203 | PASS | PASS |
...
## 7. Hotel PMS 페이지별 케이스 결과

계획서 7장의 416건을 개별 행으로 대조한 결과는 다음과 같다.

| 판정 | 건수 | 의미 |
|---|---:|---|
| PASS | 15 | 전용 시나리오로 핵심 기대 결과까지 확인 |
| PARTIAL | 33 | 일부 단계·일부 환경·한쪽 상태만 확인 |
| NOT RUN | 368 | 공통 감사 외 해당 업무 흐름 전용 테스트 미실행 |

전체 416건의 ID, 시나리오, 기대 결과, 판정, 증적과 미검증 범위는 `docs/PMS_FULL_INTEGRATION_TEST_CASE_RESULT_20260719.md`에 기록했다. 공통 스모크 PASS를 개별 기능 PASS로 환산하지 않았다.
...
- `outputs/test-critical-ui-case-map-20260719.log`
- `outputs/test-audit-visual-critical-20260718.log`
- `docs/PMS_FULL_INTEGRATION_TEST_CASE_RESULT_20260719.md`

## 10. 최종 결론

현재 공통 자동화 60개 페이지와 실행한 핵심 업무 흐름에서는 재현되는 실패가 없다. 그러나 계획서 7장의 Hotel PMS 기능 테스트는 416건 중 15건만 완결 PASS이므로 전체 통합 테스트 완료로 판정하지 않는다. 다음 단계는 PARTIAL 33건의 assertion을 보강하고 NOT RUN 368건을 우선순위 순으로 자동화·수동 실행하는 것이다. 이후 발견되는 결함은 계획서의 페이지별 케이스 ID에 연결해 재현 절차와 회귀 테스트를 함께 추가한다.
