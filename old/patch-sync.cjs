...
const newDOMContent = `window.syncGroupData = function() {
    if (!groups || !window.reservations) return;
    groups.forEach(g => {
        const groupRes = window.reservations.filter(r => (r.groupId === g.id || (r.isB2B && r.channel && r.channel.includes(g.name.split(' ')[0]))) && r.status !== 'cancelled');
        g.pickup = groupRes.length;
        if (groupRes.length > 0) {
            const hasCheckedIn = groupRes.some(r => r.status === 'checkedin');
            const allCheckedOut = groupRes.every(r => r.status === 'checkout' || r.status === 'departed');
...
console.log('Patched syncGroupData');
