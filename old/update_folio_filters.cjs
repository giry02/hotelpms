...
// 1. Replace the filter chips HTML
const oldChips = `<div class="filter-chips">
                    <button class="chip active" onclick="setStatus(this,'all')">전체</button>
                    <button class="chip" onclick="setStatus(this,'unpaid')">미수금</button>
                    <button class="chip" onclick="setStatus(this,'paid')">정산 완료</button>
                    <button class="chip" onclick="setStatus(this,'refund')">환불/취소</button>
                </div>`;
...
// We'll cycle through ['room', 'pos', 'golf', 'car']
...
const oldFilterRegex = /const filtered = folios\.filter\(f => \{[\s\S]*?return true;\s*\}\);/;
const newFilter = `const filtered = folios.filter(f => {
        const showUnpaidOnly = document.getElementById('chkUnpaid')?.checked;
        if (currentDept !== 'all' && f.dept !== currentDept) return false;
...
const oldSetStatusRegex = /function setStatus\(btn, status\) \{[\s\S]*?renderFolios\(\);\s*\}/;
const newSetDept = `function setDept(btn, dept) {
    currentDept = dept;
    document.querySelectorAll('.filter-left .chip').forEach(c => c.classList.remove('active'));
    btn.classList.add('active');
    renderFolios();
}`;
content = content.replace(oldSetStatusRegex, newSetDept);

fs.writeFileSync(path, content);
console.log('folio.html updated successfully with new filter logic.');
