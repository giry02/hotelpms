...
let brokenChunk = `        <button class="bs-option" onclick="selectFilter('diamond','Diamond')">
            <div class="bs-option-icon" style="background:rgba(96,165,250,.1);color:#60A5FA"><i class="fa-solid fa-gem"></i></div>
            <div class="bs-option-label">Diamond<div class="bs-option-desc">최상위 Tier Guest</div></div>
            <i class="fa-solid fa-check bs-option-check"></i>
    });`;

let fixedChunk = `        <button class="bs-option" onclick="selectFilter('diamond','Diamond')">
            <div class="bs-option-icon" style="background:rgba(96,165,250,.1);color:#60A5FA"><i class="fa-solid fa-gem"></i></div>
            <div class="bs-option-label">Diamond<div class="bs-option-desc">최상위 Tier Guest</div></div>
            <i class="fa-solid fa-check bs-option-check"></i>
        </button>
...
if (html.includes(brokenChunk)) {
    html = html.replace(brokenChunk, fixedChunk);
    fs.writeFileSync('E:/AI_Project/Hotel_PMS/dashboard/crm/guests.html', html);
    console.log("SUCCESS!");
} else {
    console.log("Could not find the exact broken chunk.");
}
